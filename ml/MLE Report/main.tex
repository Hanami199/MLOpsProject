...
\begin{enumerate}
    ...
    \item The one-hot encoded high-cardinality features \\ (\verb|cast|) filtered to actors appearing in $\ge 5$ productions
        \item Text features (\verb|description, listed_in, cast,| \verb|title|) processed with Sentence-BERT embeddings
        \item \verb|duration| converted to numerical format 
    \item Normalized numerical variables \verb|release_year|, \\ \verb|rating|
    ...
    \end{itemize}
     ...
     On KNN models an hyperparameter for weight the relevance of features has been added, which enhance model performance by given less importance to feature like \verb|Title| and more others like the movie genre.
...
\begin{itemize}
    ...
    \item Enabled semantic analysis of textual features \verb|description| and \verb|Title| 
    ...
    \begin{itemize}
        \item KNN (one-hot): $2356$ features (high cardinality from one-hot encoding)
        \item KNN (Embedded): $9$ features, $6$ SBERT with vector representation of $384$ elemets  $+$ $3$ normalized numerical, for a total of $2307$ numerical values per observation
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Implemented model-agnostic API supporting runtime switching:
...
\begin{itemize}
    ...
    \item \textbf{Semantic Gaps}: Plot descriptions often lacked thematic depth for accurate embedding; more detailed description would lead to a better results on the KNN Embedded model.
